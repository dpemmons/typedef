...
class Player {
 public:
  ...
  unique_ptr<Card>& get(int i) { return hand_[i]; }
  const unique_ptr<Card>& get(int i) const { return hand_[i]; }
...
int main() {
  ...
  cout << "card #0: " << dealer.get(0)->getRank() << endl;
  dealer.get(0)->setRank(SIX);
  cout << "card #0: " << dealer.get(0)->getRank() << endl;
