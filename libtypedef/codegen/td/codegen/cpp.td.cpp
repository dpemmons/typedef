...
// td::codegen::cpp::TdType member definitions

// AccessInfo member definitions

// StructDecl member definitions

// td::codegen::cpp::UserTypeDeclaration member definitions

// TmplValueDereference member definitions

// TmplFuncCall member definitions

// td::codegen::cpp::TmplExpression member definitions
...
// td::codegen::cpp::TmplItem member definitions
...
void VecUserTypeDeclarationT(std::ostream& os, const std::vector<UserTypeDeclaration>& ut, const Options& opt) {
...
std::string VecUserTypeDeclarationT(const std::vector<UserTypeDeclaration>& ut, const Options& opt) {
  std::stringstream oss;
...
void QualifiedName(std::ostream& os, const std::vector<std::string>& fqn) {
...
std::string QualifiedName(const std::vector<std::string>& fqn) {
  std::stringstream oss;
  QualifiedName(oss, fqn);
  return oss.str();
}
void QualifiedSnakeName(std::ostream& os, const std::vector<std::string>& fqn) {
...
if (key_type.td_type().is_bool_t()) {
os << "KeyType key = GetBoolFromString(m.name);";
} else 
if (key_type.td_type().is_char_t()) {
...
if (key_type.td_type().is_u8_t()) {
os << "KeyType key = GetIntFromString<std::uint8_t>(m.name);";
} else 
if (key_type.td_type().is_u16_t()) {
os << "KeyType key = GetIntFromString<std::uint16_t>(m.name);";
} else 
if (key_type.td_type().is_u32_t()) {
os << "KeyType key = GetIntFromString<std::uint32_t>(m.name);";
} else 
if (key_type.td_type().is_u64_t()) {
os << "KeyType key = GetIntFromString<std::uint64_t>(m.name);";
} else 
if (key_type.td_type().is_i8_t()) {
os << "KeyType key = GetIntFromString<std::int8_t>(m.name);";
} else 
if (key_type.td_type().is_i16_t()) {
os << "KeyType key = GetIntFromString<std::int16_t>(m.name);";
} else 
if (key_type.td_type().is_i32_t()) {
os << "KeyType key = GetIntFromString<std::int32_t>(m.name);";
} else 
if (key_type.td_type().is_i64_t()) {
os << "KeyType key = GetIntFromString<std::int64_t>(m.name);";
} else  {
...
void TmplItemsT(std::ostream& os, const std::vector<TmplItem>& v) {
...
std::string TmplItemsT(const std::vector<TmplItem>& v) {
  std::stringstream oss;
  TmplItemsT(oss, v);
  return oss.str();
}
...
void OStreamParamsList(std::ostream& os, const std::vector<AccessInfo>& params) {
os << "std::ostream& os";
...
std::string OStreamParamsList(const std::vector<AccessInfo>& params) {
  std::stringstream oss;
  OStreamParamsList(oss, params);
  return oss.str();
}
void ParamsList(std::ostream& os, const std::vector<AccessInfo>& params) {
...
std::string ParamsList(const std::vector<AccessInfo>& params) {
  std::stringstream oss;
